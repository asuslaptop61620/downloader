...
class TikTok
{
    ...
    function download($url)
    {
        ...
        if (!empty($share_data["itemInfo"]["itemStruct"]["video"]["downloadAddr"])) {
            ...
            $token['size']     = get_file_size($temp_link);
            $token['type']     = 'mp4';
            ...
            if (!empty($video_key)) {
                ...
                if (filter_var($nwm_video, FILTER_VALIDATE_URL)) {
                    ...
                    $token['url']      = $nwm_video;
                    $token['filename'] = General::orderBy('id', 'DESC')->first()->prefix . sanitize_filename( $share_data["itemInfo"]["itemStruct"]["desc"] );
                    $token['size']     = get_file_size( $nwm_video );
                    $token['type']     = 'mp4';
                    $dlLink            = url('/') . '/dl.php?token=' . encode( json_encode($token) );
                    ...
                    $video["links"][$i]["bytes"] = get_file_size($nwm_video);
        ...
        if (!empty($share_data["itemInfo"]["itemStruct"]["music"]["playUrl"])) {
            ...
            $token['url']      = $share_data["itemInfo"]["itemStruct"]["music"]["playUrl"];
            $token['filename'] = General::orderBy('id', 'DESC')->first()->prefix . time();
            $token['size']     = get_file_size( $share_data["itemInfo"]["itemStruct"]["music"]["playUrl"] );
            $token['type']     = 'mp3';
            $dlLink            = url('/') . '/dl.php?token=' . encode( json_encode($token) );
            ...
            $video["links"][$i]["bytes"] = get_file_size($video["links"][$i]["url"]);
    ...
    function get_video_key($file_data)
    {
        $key = "";
